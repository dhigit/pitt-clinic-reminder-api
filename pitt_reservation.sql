...
insert  into `tmapping`(`mid`,`patient_id`,`doctor_id`,`unfinished_high`,`unfinished_middle`,`unfinished_low`) values (1,1,1,1,0,2),(2,2,1,0,0,2),(3,3,1,0,0,0);
...
insert  into `treminder`(`rid`,`mid`,`title`,`desc`,`duration`,`created_time`,`priority`,`status`) values (0,1,'Reminder 1','The JSON-Java library is also known as org.json (not to be confused with Google\'s org.json.simple) provides us with classes that are used to parse and manipulate JSON in Java.',1,'2020-07-15 14:03:26','LOW',0),(1,1,'Reminder 2','The JSON-Java library is also known as org.json (not to be confused with Google\'s org.json.simple) provides us with classes that are used to parse and manipulate JSON in Java.',0,'2020-07-15 13:14:56','LOW',0),(4,1,'Reminder 3','The JSON-Java library is also known as org.json (not to be confused with Google\'s org.json.simple) provides us with classes that are used to parse and manipulate JSON in Java.',0,'2020-07-16 12:09:28','HIGH',0),(5,2,'Reminder 4','The JSON-Java library is also known as org.json (not to be confused with Google\'s org.json.simple) provides us with classes that are used to parse and manipulate JSON in Java.',0,'2020-07-16 23:23:42','LOW',0),(6,2,'Reminder 5','The JSON-Java library is also known as org.json (not to be confused with Google\'s org.json.simple) provides us with classes that are used to parse and manipulate JSON in Java.',0,'2020-07-16 23:33:56','LOW',0);

/* Trigger structure for table `treminder` */

DELIMITER $$

/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `trig_unfinished_count_insert` */$$

/*!50003 CREATE */ /*!50017 DEFINER = 'root'@'localhost' */ /*!50003 TRIGGER `trig_unfinished_count_insert` AFTER INSERT ON `treminder` FOR EACH ROW BEGIN
	UPDATE `tmapping` a
	SET 
		a.`unfinished_high` 	= (SELECT COUNT(*) FROM `treminder` b WHERE b.status=0 AND b.priority='HIGH' AND b.`mid`=new.mid),
		a.`unfinished_middle` 	= (SELECT COUNT(*) FROM `treminder` b WHERE b.status=0 AND b.priority='MIDDLE' AND b.`mid`=new.mid),
		a.`unfinished_low` 	= (SELECT COUNT(*) FROM `treminder` b WHERE b.status=0 AND b.priority='LOW' AND b.`mid`=new.mid)
	WHERE
		`mid` = new.mid;
    END */$$
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `trig_unfinished_count_update` */$$

/*!50003 CREATE */ /*!50017 DEFINER = 'root'@'localhost' */ /*!50003 TRIGGER `trig_unfinished_count_update` AFTER UPDATE ON `treminder` FOR EACH ROW BEGIN
	UPDATE `tmapping` a
	SET 
